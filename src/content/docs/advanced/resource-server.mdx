...
```toml
[dependencies]
authkestra-resource = "0.9"
authkestra-axum = { version = "0.13", features = ["resource"] }
# Or if using Actix:
# authkestra-actix = { version = "0.13", features = ["resource"] }
```
...
```toml
[dependencies]
authkestra-resource = { version = "0.13", default-features = false, features = ["rustls-no-provider"] }
```
