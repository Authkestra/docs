...
authkestra-axum = { version = "0.9", features = ["resource"] }
# Or if using Actix:
# authkestra-actix = { version = "0.9", features = ["resource"] }
```
...
```toml
[dependencies]
authkestra-resource = { version = "0.9", default-features = false, features = ["rustls-no-provider"] }
```
